# VBA_COM
Uses a class module called ClRS232 -> see code for details on who created it
Option Explicit

'-------------------------------------------------------------------------------
' Basic Class Module created from (modComm by David M. Hitchner [see below])
'
' Version 1.01 by Brendan Lambe 23 Oct 2012
'
' Parameters :
' BaudRate              - (R/W) Default value = 115200
...
' InputBufferSize       - (R/W) Default value = 1024
...
#If VBA7 Then
    ...
    Private Declare PtrSafe Function GetOverlappedResult Lib "kernel32" _
        (ByVal hFile As LongPtr, lpOverlapped As OVERLAPPED, _
    ...
    Private Declare PtrSafe Function SetupComm Lib "kernel32" _
        (ByVal hFile As LongPtr, ByVal dwInQueue As Long, ByVal dwOutQueue As Long) As Long
    
    Private Declare PtrSafe Function WriteFile Lib "kernel32" _
        (ByVal hFile As LongPtr, lpBuffer As Any, _
...
Private clInBufferSize As Long
...
#If VBA7 Then
Private Function SetCommErrorEx(strFunction As String, lngHnd As LongPtr) As Long
#Else
...
#If VBA7 Then
    ...
    Dim i As Integer, bRead() As Byte, strRdBuffer As String
    
    ReDim bRead(clInBufferSize)
    
    lngRdStatus = ReadFile(udtPorts(intPortID).lngHandle, bRead(0), _
            lngRdSize, lngBytesRead, udtCommOverlap)
    
    If lngRdStatus <> 0 Then
        For i = 0 To lngBytesRead - 1
            strRdBuffer = strRdBuffer & Chr(bRead(i))
        Next i
    End If

#Else
        Dim strRdBuffer As String
        strRdBuffer = Space(clInBufferSize)
...
#If VBA7 Then
    Dim LenVal As Long
    Dim bRead(1024) As Byte

    For LenVal = 0 To lngsize - 1
        bRead(LenVal) = Asc(Mid(strData, LenVal + 1, 1))
    Next LenVal

    ' Output the data.
    lngWrStatus = WriteFile(udtPorts(intPortID).lngHandle, bRead(0), lngsize, _
...
Property Let InputBufferSize(T As Long)
    clInBufferSize = T
End Property

Property Get InputBufferSize() As Long
